...
public class YMCAController {
    ...
    public String calculate(@ModelAttribute("ymcaResult") YMCAResult ymcaResult, @ModelAttribute("lastYmca") YMCA ymca,
        ...
        if (session.getAttribute("userid") != null) {
            ...
            List<YMCA> ymcas = ymcaService.getYMCAResultsForUser(userid);
            if (ymcas.size() > 0) {
    ...
    public ModelAndView calculateYMCA(@ModelAttribute("ymcaResult") YMCAResult ymcaResult, HttpSession session) {
        double ymcaValue = calculateYmcaValue(ymcaResult.getWeight(), ymcaResult.getWaistCircumference(),
                ymcaResult.getSex());
        ...
        if (session.getAttribute("userid") != null) {
            ...
            ymca.setUserId(userid);
            ymca.setTimestamp(Timestamp.from(Instant.now().truncatedTo(ChronoUnit.MINUTES)));
    ...
    private double calculateYmcaValue(double weight, double waistCircumference, String sex) {
        // Zawartość tkanki tłuszczowej (kobiety) = ((1.634 x obwód pasa [cm] – 0.1804 x
        // masa ciała [kg] - 76.76 ) / 2,2 * masa ciała [kg]) x 100
        // Zawartość tkanki tłuszczowej (mężczyźni) = ((1.634 x obwód pasa [cm] – 0.1804
        // x masa ciała [kg] - 98.42 ) / 2,2 x masa ciała [kg]) x 100
